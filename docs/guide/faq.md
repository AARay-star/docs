...
1. 在使用V2的烧录工具，选择完屏幕信息后按回车键，然后一直卡着不出固件版本信息，等待一段时间卡死，报错闪退。
2. 选择完下载镜像之后卡死不出后面的进度条，然后报错闪退。

**可能原因**：程序正下载屏幕固件清单 manifest.json，但它走了本机代理，代理中断了HTTPS连接。

#### 解决方案
先关闭刷机程序，打开 PowerShell，完整复制运行：

```PowerShell
$env:HTTP_PROXY=$null
$env:HTTPS_PROXY=$null
$env:ALL_PROXY=$null
$env:NO_PROXY='epflash.iccmc.cc'
& 'D:\Tongxing\软件2.x版本（最新，下这个）\刷机程序epass_flasherV2.exe'
```

后面那个改成自己电脑刷机程序的路径。
这只会让这一次启动的刷机程序绕过代理，不会永久修改你的代理设置。
